...
def get_db_connection():
    # database disimpan di path absolut home /home/pi/
    db_path = '/home/pi/aquaponics_history.db'
    # Jika tidak berjalan di Pi (Windows testing), simpan di direktori lokal
...
def init_db():
    ...
    # Check if empty, seed with 30 days of realistic dummy data
        ...
        if self.path == "/api/status":
            ...
            try:
                ...
                if range_param == 'harian':
                    # Ambil data 24 jam terakhir, dirata-rata per jam
                    cursor.execute('''
                    ...
                    # Ambil data 7 hari terakhir, dirata-rata per hari
                    cursor.execute('''
                    ...
                    # Ambil data 30 hari terakhir, dirata-rata per hari
                    cursor.execute('''
                        SELECT strftime('%d/%m', datetime(timestamp, 'localtime')) as date_str,
                               AVG(temp_w) as avg_temp_w,
                               AVG(tds) as avg_tds
...
def upload_sensor_data(sensor_data_json):
    ...
    try:
        ...
        # Log ke SQLite database sekali setiap 1 menit
...
def push_to_firebase(sensor_data):
    """Mengunggah data sensor ke Firebase Realtime Database via REST API"""
...
def apply_remote_command(cmd):
    """Memproses perintah kontrol jarak jauh yang diterima dari Firebase"""
    global config_data, latest_sensor_data
    try:
...
def listen_firebase_control_loop():
    """Mendengarkan perubahan kontrol pada path /control di Firebase Realtime Database"""
...
def main():
    ...
    # Jalankan Firebase Listener di thread background
